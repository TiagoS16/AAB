...
class MyGraph:
    ...
    def search_hamiltonian_path_from_node(self, start):
        ...
        visited = {start:0}
        path = [start]
        while len(path) < len(self.get_nodes()): #tamanho do path menor que o numero de vertices
    ...
    def check_balanced_node(self, node):  # verifica se o numero de arcos de entrada é igual ao numero de arcos de saida para um dado no
        return self.in_degree(node) == self.out_degree(node)  # verifica os graus do no
        
    def check_balanced_graph(self):  # verifica se o grafo é balanceado --> se todos os nos forem balanceados
        for n in self.graph.keys():  # iterar o numero de nos
            if not self.check_balanced_node(n):  # caso um deles não seja balanceado o grafo deixa de ser euleriano
                return False
        return True
    
    def check_nearly_balanced_graph(self):  # verifica se o grafo tem um circuito euleriano --> max 2 nos semi-balanceado (a diferença entre os graus de entrada e saida tem de ser 1 e -1 respetivamente)
        res = None, None
        for n in self.graph.keys():  # iterar os nos do grafo
            indeg = self.in_degree(n)  # verificar o numero de graus/arcos de entrada
            outdeg = self.out_degree(n)  # verificar o numero de graus/arcos de saida
            if indeg - outdeg == 1 and res[1] is None:  # caso os arcos de entrada menos os de saida seja 1
                res = res[0], n
            elif indeg - outdeg == -1 and res[0] is None:  # caso os arcos de entrada menos os de saida seja -1
                res = n, res[1]
            elif indeg == outdeg:  # caso sejam iguais é um no balanceado
                pass
            else:
                return None, None
    ...
    def eulerian_cycle(self):
        if not self.is_connected() or not self.check_balanced_graph():  # verifica se é euleriano (condições suficientes)
            return None  # caso não seja para a função aqui
        arcos = list(self.get_edges())  # uma lista de tuplos com os nos de origem e destino
        res = []
        while arcos:  # quando a lista de arcos estiver vazia o ciclo para
