...
class DeBruijnGraph (MyGraph):
    ...
    def create_deBruijn_graph(self, frags):
        for seq in frags:
            suf = suffix(seq)
            self.add_vertex(suf) # adiciona o vertice do sufixo
            pref = prefix(seq)
            self.add_vertex(pref) # adiciona o vertice do prefixo
            self.add_edge(pref, suf) # adiciona as ligacoes entre os vertices prefixo e sufixo
